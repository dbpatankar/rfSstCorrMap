...
class App:
    ...
    def __init__(self, master):
        ...
        self.nc_season_combo = Combobox(self.topright_frame, textvariable=self.nc_season, values=["DJF", "MAM", "JJA", "SON"])
    ...
    def plotData(self, event):
        ...
        #n = nc.groupby('time.year').apply(np.mean)
        ...
        if hasattr(self, 'c'):
            ...
            pass
        mp = Basemap(llcrnrlon=0, llcrnrlat=-90,urcrnrlon=358,urcrnrlat=90)
        mp.drawcoastlines(ax = self.a, linewidth=0.5)
        self.ax = self.a.pcolormesh(n.lon, n.lat, n.cca)
        self.c = self.f.colorbar(self.ax)
        self.a.set_xticks(np.arange(0, 358.1, 30))
        self.a.set_yticks(np.arange(-90.0, +90.1, 10))
        self.canvas.draw()
